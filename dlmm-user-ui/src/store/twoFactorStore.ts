...
export const twoFactorStore = {
  ...
  __resetForTests(): void {
    // G-20 backend swap removed localStorage entirely (the store now
    // hydrates from /users/me/2fa/status). The cache=null + notify
    // pattern (preserved from 06f964c) keeps tests isolated; we also
    // reset lastKnownTotal so the recovery-codes-count carry-over
    // between tests doesn't leak.
    cache = null
    lastKnownTotal = 0
    notify()
  },
