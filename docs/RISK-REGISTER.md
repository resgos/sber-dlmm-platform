...
| 4 | **Liquibase changeset model is "MARK_RAN"-hack** — future schema changes won't apply on existing DBs | 4 | 4 | 16 | **CLOSED (Sprint 3 #3.8).** Re-analysis showed risk applies only to *new CREATE TABLE* changesets that copy-paste the preCondition wrapper. ALTER changesets and CREATE-without-preConditions work cleanly on both greenfield and existing DBs. Documented in `docs/DB-MIGRATION-CONVENTION.md` + README in every `db/changelog/`. Includes anti-pattern + PR checklist. Full split (drop init-db.sql, seed via CommandLineRunner) parked as Sprint 4+ option. | Backend lead | Convention enforced via PR review. |
