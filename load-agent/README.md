...
[Проверки корректности после нагрузки (`assert`)](#проверки-корректности-после-нагрузки-assert)
...
## Проверки корректности после нагрузки (`assert`)

Быстро — не значит верно: нагрузка может «держать RPS», но при этом **терять или портить данные**.
Блок `assert` прогоняет проверочные SQL-запросы к витрине **после нагрузки** (и до `teardown`, который
чистит данные) — счётчики сошлись, нет дублей, суммы бьются. Любое расхождение = **FAIL**. Это ровно
твои грабли: prefix-sum silent leak на границе retention, «быстро, но потеряли строки».

```jsonc
{
  "kind": "sql",  // или "pipeline" (проверка витрины конвейера)
  ...
  "assert": [
    { "name": "все вставки на месте", "sql": "select count(*) from orders", "expect": { "minValue": 1000 } },
    { "name": "нет дублей по ключу",   "sql": "select count(*)-count(distinct id) from orders", "expect": { "value": "0" } },
    { "name": "суммы дебет=кредит",     "sql": "select case when sum(dt)=sum(ct) then 0 else 1 end from ledger", "expect": { "value": "0" } },
    { "name": "нет строк вне окна",     "sql": "select count(*) from t where ts < now()-interval '1 day'", "expect": { "value": "0" } }
  ]
}
```

- Каждая проверка — SQL, возвращающий **скаляр** (первая ячейка). `expect` — ровно ОДНО из:
  `value` (равно), `minValue`/`maxValue` (число ≥/≤), `minRows` (столько строк), `notEmpty`.
- Выполняется one-shot через клиента СУБД (psql: `-t -A -v ON_ERROR_STOP=1`) — ошибка запроса = провал
  проверки (не молчит). Для `kind:"sql"` берётся `sql`-драйвер, для `kind:"pipeline"` — `verify.sql`.
- Отчёт: секция **«ПРОВЕРКИ КОРРЕКТНОСТИ (assert)»** с полученным vs ожидаемым; любой FAIL валит вердикт
  (exit 2). Работает с `driver:"psql"` (для другого клиента задайте `sql.assertFlags`).
- Cross-node консистентность Ignite — отдельными проверками к каждой ноде (разные `command`/сценарии).
- ⚠ `{{список}}` в assert-запросе подставит ОДНО случайное значение — для «все N id на месте» пишите
  агрегат (`count(*)`, `count(distinct …)`), а не `… where id={{id}}` (иначе проверите один случайный id).
