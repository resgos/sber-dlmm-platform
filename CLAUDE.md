...
### Data + migrations
Single Postgres DB `dlmm`. Schema is bootstrapped by `docker/init-db.sql`; Liquibase runs `validate-on-migrate` and JPA `ddl-auto: validate`, so **a schema change needs a Liquibase changeset, not just an entity edit**. Per-service changesets wrap `createTable` in `<preConditions onFail="MARK_RAN">` so they coexist with `init-db.sql` (a known tactical hack — see `docs/DB-MIGRATION-CONVENTION.md`). Demo data is layered numbered seed scripts `docker/0X-*.sql`; the compose entrypoint auto-mounts only `01-04`; the rest (`05-12`) touch Liquibase-created tables, so they run after the stack is up via **`scripts/seed-demo.sh`** (idempotent, per-step `demo_seed_log` markers — see Operational gotchas).

**Amount scale (#14):** every token amount is a raw integer (`long`/BIGINT) where 1 unit = 10⁻⁴ token (uniform 4 "platform decimals"); the backend never applies a token's `decimals` column. The user-ui converts raw↔human at the API boundary (`dlmm-user-ui/src/api/scale.ts` — `scale*` per response type + `toRaw` on requests) so fractional amounts (e.g. 0.5 SBTC) round-trip instead of the swap engine flooring sub-1-token output to 0. A **uniform** scale leaves prices (Y/X ratios) unchanged — scale ONLY quantities, never price/bps/fee-growth/ratios. `docker/11-seed-rescale-amounts-1e4.sql` applies the ×10⁴ to demo data (idempotent, flag-guarded). Pinned by `scale.test.ts`. `total/circulating_supply` is deliberately left unscaled on both sides (undisplayed, unused in trade flows).
...
- **`mvn -pl <module> package` can ship a STALE jar** when only a dependency (e.g. `dlmm-common`) changed: the module's own sources are up-to-date, so the jar isn't rebuilt — BUILD SUCCESS, but `target/*.jar` keeps its old mtime/bytes and a hot-deploy silently deploys old code (bit us 2026-06-12). After changing dlmm-common, build dependents with `clean package`, and sanity-check `ls -la target/*.jar` mtime before `docker cp`.
- **Seed-script idempotency:** `05-seed-volume-refresh.sql` is safe and **must** re-run within ~24h of a demo (the pool-engine scheduler ages swaps out of the 24h window, drifting volume/APY to 0). `06-seed-tvl-rescale.sql` and `08-seed-balance-rescale.sql` are **NON-idempotent — never re-run** (they divide by a constant). `07` and `09` are safe/idempotent. Order after `down -v`: 05, 06, 07, 08, 09, 10, 11. `10-seed-reconcile-bin-invariant.sql` (F-12) and `11-seed-rescale-amounts-1e4.sql` (#14 fractional amounts, ×10⁴) are both **idempotent** (flag/marker-guarded) — apply them after the 06/08 divides (11 multiplies). `12-seed-farming-rewards.sql` (LP-farming demo data: reward configs + accrued rewards) runs last, idempotent via `ON CONFLICT`. **`scripts/seed-demo.sh` automates this whole 05→12 sequence** after a fresh `docker compose up` — a `demo_seed_log` marker table makes it resumable and guarantees the non-idempotent 06/08 run exactly once. (`scripts/redeploy-backends.sh` hot-redeploys the service jars; `redeploy-frontends.sh` the UI dist.)
